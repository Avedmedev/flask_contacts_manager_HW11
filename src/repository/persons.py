...
def get_persons_owner(owner_id):
    contacts = (
        session.query(models.Person).filter(models.Person.owner_id == owner_id).all()
    )
...
def delete_contact_owner(owner_id, contact_id):
    contact = (
        ...
        .filter(
            and_(models.Person.id == contact_id, models.Person.owner_id == owner_id)
        )
        .first()
    )
...
def update_person(contact_id, owner_id, first_name, last_name, phone_number, email):
    ...
    session.commit()
    phone = (
        session.query(models.Phone).filter(models.Phone.person_id == contact_id).first()
    )
    phone.phone_number = phone_number
    session.commit()
    em = (
        session.query(models.Email).filter(models.Email.person_id == contact_id).first()
    )
    em.email = email
    session.commit()
