...
def login():
    ...
    if request.method == 'POST':
        ...
        if owner is None:
            return render_template('pages/login.html', messages={'err': 'Invalid credentials. Go to administration'})
...
def contacts_add():
    ...
    if request.method == 'POST':
        try:
            ...
            return render_template('pages/add_contact.html', auth=auth, messages=err.messages)
        first_name = request.form.get('first_name')
        last_name = request.form.get('last_name')
        phone_number = request.form.get('phone_number')
        email = request.form.get('email')

        persons.add_contact(session['username']['id'], first_name, last_name, phone_number, email)
...
def edit_picture(contact_id):
    ...
    if request.method == 'POST':
        try:
            ...
            return render_template('pages/edit.html', auth=auth, contact=person, messages=err.messages)
        ...
        persons.update_person(contact_id, owner_id, first_name, last_name, phone_number, email)
        flash('Successfully')
        return redirect(url_for('contacts'))
